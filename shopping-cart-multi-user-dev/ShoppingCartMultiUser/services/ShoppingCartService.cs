                ...
                if (c.GetClientId() == clientId)
                {
                    if (c.GetShoppingCart().Count != 0)
                    {
                        foreach (var cartItem in c.GetShoppingCart())
                        {
                            ...
                            if (product != null)
                            {
                                StringBuilder shoppingCartContent = new("Your shopping cart:\n");

                                return shoppingCartContent.AppendLine($"{product.Name} : {cartItem.Quantity}").ToString();
                            }
                        }
                    }
                }

            return "There is no products in your shopping cart!";
        }
